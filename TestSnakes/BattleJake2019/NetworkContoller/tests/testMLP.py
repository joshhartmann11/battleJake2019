...
class testMLP(unittest.TestCase):
    ...
    """
     ...
     .d8888b. 8888888 .d8888b.        8888888b.  8888888b.  8888888 888b     d888 8888888888
    d88P  Y88b  888  d88P  Y88b       888   Y88b 888   Y88b   888   8888b   d8888 888
    Y88b.       888  888    888       888    888 888    888   888   88888b.d88888 888
     "Y888b.    888  888              888   d88P 888   d88P   888   888Y88888P888 8888888
        "Y88b.  888  888  88888       8888888P"  8888888P"    888   888 Y888P 888 888
          "888  888  888    888       888        888 T88b     888   888  Y8P  888 888s
    Y88b  d88P  888  Y88b  d88P       888        888  T88b    888   888   "   888 888
     "Y8888P" 8888888 "Y8888P88       888        888   T88b 8888888 888       888 8888888888
    ...
    def _test_propigate_unit_synapses(self, layers):
        ...
        for i in range(len(layers) - 1):
            output = test.sigmoid(output*len(test.synapses[i]))

        actual = test.propigate(input, test.synapses)

        for i in range(len(actual.tolist()[0])):
            self.assertAlmostEqual(actual.tolist()[0][i], output)

    def test_propigate(self):
        self._test_propigate_unit_synapses([2, 3, 2])
        self._test_propigate_unit_synapses([25, 64, 21, 2, 50])
    ...
    def _test_back_propigate_unit_synapses(self):
        pass

    def _test_back_propigate_negative_change(self, layers):
        test = MLP(layers)
        inputNumber = 5
        input = [inputNumber] * layers[0]
        output = [0] * layers[-1]
        layers = test.propigate(input, test.synapses, allLayers=True)
        initialGuess = layers[-1]
        change = test.back_propigate(output, layers)

        for s in change:
            for x in range(len(s)):
                for y in range(len(s[0])):
                    self.assertLess(s[x, y], 0, "Change should be negative")


    def _test_back_propigate_positive_change(self, layers):
        test = MLP(layers)
        inputNumber = 5
        input = [inputNumber] * layers[0]
        output = [1] * layers[-1]
        ...
        for s in change:
            for x in range(len(s)):
                for y in range(len(s[0])):
                    self.assertGreater(s[x, y], 0, "Change should be positive")


    def test_back_propigate(self):
        self._test_back_propigate_positive_change([2, 3, 2])
        self._test_back_propigate_negative_change([2, 3, 2])
        self._test_back_propigate_positive_change([2, 3, 2])
