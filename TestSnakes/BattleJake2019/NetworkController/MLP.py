...
class MLP:
    ...
    def pprint(self, thing, header=None):
        if self.verbose:
            print("_" * 100)
            print("[{0}: MLP]".format(time.strftime("%Y-%m-%d %H:%M")))
            print("")
            if header:
                print(header)
                print("")
    ...
    # Return the cumulative error of come tests
    def test_network(self, inputs, outputs):
        numSamples = len(inputs)
        cumError = 0
        for i in range(len(inputs)):
            output = self.propigate(inputs[i], allLayers=False)
            cumError += np.abs(outputs[i] - output)/4
        return np.sum(cumError/numSamples)
    ...
    def back_propigate(self, actual, layers):
        ...
        #print("Compiled Error: " + str(np.sum(np.abs(error)/4)))
        #print("*"*50)
        ...
        for i in reversed(range(len(self.synapses))):
            ...
            if i == 2:
                self.pprint("Change from da source:\n{}".format(change))
    ...
    def learn(self, input, output, lMul=1.1, lRate=0.0005):
        ...
        layerOutputs = self.propigate(input, allLayers=True)
    ...
    def batch_learn(self, inputs, outputs, lMul=1.1, lRate=0.0005):
        ...
        for w in range(len(self.synapses)):
            ...
            layerMul = math.pow(lMul, len(self.synapses) - (w+1))
            if w == (len(self.synapses)-1):
                self.pprint("Change from da learn\n{}".format(change[-w-1]))
            self.synapses[w] = np.add(self.synapses[w], change[-w-1] * lRate * layerMul)

if __name__ == "__main__":
    test = MLP([2, 120, 200, 4], verbose=True)
    for i in range(100000):
        test.batch_learn([np.array([1, 1])], [np.array([0.5, 0, 0.3, 0.1])])
    #import subprocess
