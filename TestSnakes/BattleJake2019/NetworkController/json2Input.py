...
# FIXME: GET THE X, Y, DATA YOU LAZY BEAR
def encode_xy_list(xyList, x=20, y=20, onValue=1):
    output = np.zeros(x*y)
    for xy in xyList:
        index = xy["x"] + x*xy["y"]
        output[index] = 1
...
def main():
    ...
    args = get_input(sys.argv)
    files = os.listdir(args.input)
    numFiles = len(files)

    print("Finished Listing Files...")

    files = [os.path.abspath(os.path.join(args.input, f)) for f in files]
