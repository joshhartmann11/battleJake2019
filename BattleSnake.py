...
class SnakeAIInterface():
    def move():
        raise ImplementationError("\"move\" is not implemented")
...
if __name__ == "__main__":
    ...
    for i in range(1000):
        ...
        s.add_snake(Snake("MitchellNursey", color=COLORS["green"]), MitchellNursey())
        try:
            gameWinners.append(s.start_game(speed=100, outputBoard=False, debug=False))
        except Exception as e:
            print("FAILURE: ", e)
 
    homies = ["Jake2018", "SimpleJake", "MitchellNursey", "SajanDinsa", "MimicMitchellNursey"]
    for h in homies:
